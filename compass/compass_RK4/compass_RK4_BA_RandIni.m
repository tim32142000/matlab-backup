...
b1 = 36.0 ;
b2 = 95.0 ;
n_ini = 1 ; % number of initial conditions
w_min = -1.75 ; % minimum of omega in (radian/(2*pi) )
w_max = 1.75 ; % maximum of omega in (radian/(2*pi) )
rng(33) % seed of Random Number Generator
theta0 = rand(n_ini,1)-0.5 ; % initial angle in (radian/(2*pi) )
...
coss = 10 ; % condition of successive satisfying times
cur_ssa = 0 ; % current successive satisfying for a
cur_ssb = 0 ; % current successive satisfying for b

theta_ave_n1a = 0.0 ; % a condition. from other program, in radian
