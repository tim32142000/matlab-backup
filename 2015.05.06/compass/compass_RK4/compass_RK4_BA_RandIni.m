% Calculating Basin of Attraction(BA) with Monte Carlo inputing initial condition by RK4 method
...
b1 = 36.0 ;
b2 = 103.2 ;
n_ini = 1 ; % number of initial condition sets
w_min = -1.75 ; % minimum of omega in (radius/(2*pi) )
w_max = 1.75 ; % maximum of omega in (radius/(2*pi) )
rng(33) % seed of Random Number Generator
theta0 = rand(n_ini,1)-0.5 ; % initial angle in (radius/(2*pi) )
w0 = rand(n_ini,1).*(w_max-w_min)+w_min ;
...
theta_ave_n1a = 0.0 ; % a condition. from other program, in radius
theta_ave_n1b = 0.0 ; % b condition

wetau = w_ext*tau ;
% nthw = 0 ; % count of initial condition
...
for k = 1:n_ini % loop change initial condition
    ...
    while (n <= tot_ts_max) && (leave_while_n == 0)
        n = n+1 ;
        rkdth1 = w(n) ;
        rkdw1 = -gamma*w(n) - b1*sin(theta(n) ) + b2*cos(theta(n) )*cos( n*wetau) ; % have been corrected
        rkdth2 = rkdth1 + 0.5*tau*rkdw1 ;
        rkdw2 = -gamma*rkdth2 - b1*sin(theta(n) + 0.5*rkdth1*tau) + b2*cos(theta(n) + 0.5*rkdth1*tau)*cos( (n+0.5)*wetau) ; % have been corrected
        rkdth3 = rkdth1 + 0.5*tau*rkdw2 ;
        rkdw3 = -gamma*rkdth3 - b1*sin(theta(n) + 0.5*rkdth2*tau) + b2*cos(theta(n) + 0.5*rkdth2*tau)*cos( (n+0.5)*wetau) ; % have been corrected
        rkdth4 = rkdth1 + tau*rkdw3 ;
        rkdw4 = -gamma*rkdth4 - b1*sin(theta(n) + rkdth3*tau) + b2*cos(theta(n) + rkdth3*tau)*cos( (n+1)*wetau) ; % have been corrected
        ...
        if(mod(n,b2_per_ts)==0 )
            ...
            if (jj>20)
                if  (abs( mean(theta_ave(jj-2:jj) ) - theta_ave_n1a) < 10^-3) % condition for a
                    cur_ssa = cur_ssa + 1 ;
                    if (cur_ssa >= coss)
                ...
                elseif (abs( mean(theta_ave(jj-4:jj) ) - theta_ave_n1b) < 10^-3) % condition for b
                    cur_ssb = cur_ssb + 1 ;
                    if (cur_ssb >= coss)
